$.ajaxSetup({
  ...
  table = $('#Etudiant_table').DataTable({ 
    ...
    function add() {
        $('#ajoutEtudiant').modal('toggle');   
      }
    $("#add_etudiant").submit(function(event){
    ...
    function supp(id){
      ...
      function editform(id) {
        ...
        .done(function(response){
          ...
          $('#etumodif').val(response[0].nom);
          $('#groupemodif').val(response[0].groupe);
      ...
      $("#edit_etudiant").submit(function(event){
        event.preventDefault();
        var post_url = $(this).attr("action");
        var request_method = $(this).attr("method");
        var form_data = $(this).serialize();
