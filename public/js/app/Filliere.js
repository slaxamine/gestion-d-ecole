...
table = $('#Filliere_table').DataTable({ 
  ...
  function editform(id) {
    ...
    .done(function(response){
      ...
      $('#fillieremodif').val(response[0].name);
  ...
  $("#edit_filliere").submit(function(event){
...
toastr.options = {
 ...
 function add() {
    $('#ajoutFilliere').modal('toggle');
    
  }


  $("#add_filliere").submit(function(event){
    event.preventDefault();
    var post_url = $(this).attr("action");
    var request_method = $(this).attr("method");
    var form_data = $(this).serialize();
